...
The **Azure IoT TypeEdge** introduces a strongly typed flavor of the inherently loosely coupled vanila [Azure IoT Edge](https:/azure.microsoft.com/en-us/services/iot-edge/).
...
To setup your development environment for **TypeEdge** you will need to:
 ...
 2. **Install the latest [.NET Core](https://www.microsoft.com/net/learn/get-started)**
 3. **Install your favorite IDE ([VS Code](https://code.visualstudio.com/download) or [VS 2017](https://www.visualstudio.com/downloads/))**

    These three steps will get you  ready to build your first TypeEdge application.

- **Temporary step - private NuGet packages**:
Temporarily, and until GA, the required NuGet packages will be distributed through this code repo. After GA, these packages will be available in the www.nuget.org. To get the private packages, just clone this repo:
    ```
    git clone https://github.com/paloukari/TypeEdge.git
    ```
...
1. Install the TypeEdge .NET Core solution template. One level above the repo clone type:
    ```
    dotnet new -i TypeEdge.Application::* --nuget-source TypeEdge\Example\build
    ```
...
This module has a strongly typed outout called ***Output*** and the messages type is ***SensorModuleOutput***. Similarly, it has a module twin called ***Twin*** with type ***SensorModuleTwin***
